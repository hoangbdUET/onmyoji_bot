...
class Fighter(GameScene):
    ...
    def __init__(self, emyc=0, hwnd=0):
        '''
        initialization
            : param emyc=0: Point blame setting: 0-no point blame
    ...
    def check_end(self):
        ...
        Check whether it is finished
            :return: Return 0 on the victory page; return 1 on the reward page
        '''
    ...
    def check_times(self):
        ...
        if(self.run_times == self.max_times):
            if(self.end_operation == 0):
                self.log.warning('Close the script (the number of times is full)...')
                self.run = False
                os._exit(0)
            elif(self.end_operation == 1):
                self.log.warning('Close the game (the count is full)...')
                self.yys.quit_game()
                self.log.warning('Close the script (the number of times is full)...')
                self.run = False
                os._exit(0)
    ...
    def get_reward(self, mood, state):
        ...
        Settlement processing
            ...
            :param state: The status of the previous step. 0-Successful battle page; 1-Receive reward page
        '''
        ...
        self.log.warning('Click settlement failed!')
    ...
    def mitama_team_click(self):
        '''
        Yushun marks one's own way god
        '''
    ...
    def click_until(self, tag, img_path, pos, pos_end=None, step_time=0.8, appear=True):
        ...
        In a certain period of time, click the mouse in the background until a certain picture appears or disappears
            ...
            :param pos_end=None: (x,y) If pos_end is not empty, the mouse clicks a random position in the area with pos as the upper left corner coordinate pos_end as the lower right corner coordinate
            :step_time=0.5: Query interval
            :appear: The picture appears or disappears: Ture-appears; False-disappears
            :return: Return True on success, exit the game on failure
        '''
    ...
    def slide_x_scene(self, distance):
        ...
        Horizontal sliding scene
            :return: Return True for success; Return False for failure
        '''
